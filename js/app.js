...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `LCY Video Pro - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            window.history.pushState(
                { search: query },
                `搜索: ${query} - LCY Video Pro`,
                `/s=${encodedQuery}`
            );
            // 更新页面标题
            document.title = `搜索: ${query} - LCY Video Pro`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'LCY Video Pro-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'LCY Video Pro-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'LCY Video Pro-Settings_' + times + '.json');
}
